...
//  ToDo:
//
//  - Fix remove from file system
//  - Add https://github.com/aplourde/mp4v2-cocoa
//		- Add Reading all tags
//		- Remove all tags
//		- [Set tags individually]
//  - Fix umlaute in display commands and searching
//  - Optimizing iTunes Search
//
#import "Movie.h"
#import "XMLParser.h"
#import "iTunesAPI.h"
	...
	@autoreleasepool {
		...
		for (NSString *m4vFile in m4vFiles) {
			...
			if (!ignoreTags) {
				...
				//				currentMovie.title = mp4File.metadata.name;
				//				currentMovie.comments = mp4File.metadata.comments;
				//				currentMovie.genres = [NSArray arrayWithObjects: mp4File.metadata.genre, nil];
				//				currentMovie.releasedate = mp4File.metadata.releaseDate;
				//				currentMovie.shortdescription = mp4File.metadata.shortDescription;
				//				currentMovie.plot = mp4File.metadata.longDescription;
				//				currentMovie.hd = mp4File.metadata.hd;
				//				currentMovie.contentAdvisoryRating = mp4File.metadata.contentRating;
				//				//currentMovie.type = mp4File.metadata.type;
				//				currentMovie.contentID = mp4File.metadata.contentId;
				//				currentMovie.artwork = mp4File.metadata.artwork;
				//				currentMovie.studio = mp4File.metadata.studio;
